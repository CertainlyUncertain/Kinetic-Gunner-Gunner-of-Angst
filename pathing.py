...
class Pathing:
    ...
    def tick(self, dt):
        ...
        if len(self.paths['yaw']) > 0:
            self.ent.yawRate = self.paths['yaw'][self.indices['yaw']].tick(dt)
        ...
        if len(self.paths['pitch']) > 0:
            self.ent.pitchRate = self.paths['pitch'][self.indices['pitch']].tick(dt)
        ...
        if len(self.paths['speed']) > 0:
            self.ent.acceleration = self.paths['speed'][self.indices['speed']].tick(dt)
